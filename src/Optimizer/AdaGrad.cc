...
NEURAL_NETWORK::AdaGrad::AdaGrad(double learning_rate, double decay, double epsilon) : Optimizer(learning_rate, decay)
{
	epsilon_ = epsilon;
}
...
void NEURAL_NETWORK::AdaGrad::UpdateParameters(NEURAL_NETWORK::LayerDense& layer)
{
	...
	weight_cache_update = NEURAL_NETWORK::Helpers::MatrixSquare(layer.GetDWeights());
	bias_cache_update = NEURAL_NETWORK::Helpers::MatrixSquare(layer.GetDBiases());
