...
void NEURAL_NETWORK::MaxPooling::forward(const Eigen::Tensor<double, 4>& inputs, bool training)
{
...
void NEURAL_NETWORK::MaxPooling::backward(const Eigen::Tensor<double, 4>& dvalues)
{
    ...
    // Use tensor dvalues directly (no conversion)
    int window_size = pool_size_ * pool_size_ * input_channels_;
    int num_windows = actual_batch_size * output_height_ * output_width_;
    Eigen::MatrixXd d_col = Eigen::MatrixXd::Zero(window_size, num_windows);
    ...
    for (int w = 0; w < num_windows; w++)
    {
        ...
        for (int c = 0; c < input_channels_; c++)
        {
            ...
            d_col(d_col_idx, w) = dvalues(b, h, ow, c);
        }
    }
