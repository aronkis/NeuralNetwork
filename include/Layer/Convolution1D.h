#ifndef __CONVOLUTION_1D_H__
#define __CONVOLUTION_1D_H__
...
{
	class Convolution1D : public LayerBase
	{
	public:
		Convolution1D(int number_of_filters, int filter_length, int input_length,
					  int input_channels, int padding, int stride,
		...
		const Eigen::Tensor<double, 3>& GetWeightsTensor() const;
	...
	private:
		...
		Eigen::Tensor<double, 3> tensor_output_;
		Eigen::MatrixXd output_;

		Eigen::MatrixXd d_input_;
		Eigen::Tensor<double, 3> d_input_tensor_;
		Eigen::Tensor<double, 3> d_weights_;
		Eigen::VectorXd d_biases_;

		Eigen::Tensor<double, 3> weight_momentums_;
		Eigen::VectorXd bias_momentums_;
		Eigen::Tensor<double, 3> weight_caches_;
		...
		void InputMatrixToTensor(const Eigen::MatrixXd& matrix, int batch_size,
								 int length, int channels);

		Eigen::MatrixXd InputTensorToMatrix(const Eigen::Tensor<double, 3>& tensor) const;

		void WeightsToTensor(const Eigen::MatrixXd& weights_matrix);
		Eigen::MatrixXd WeightsToMatrix() const;

		Eigen::MatrixXd WeightsToMatrixFromTensor(const Eigen::Tensor<double, 3>& tensor) const;
		void MatrixToWeightsTensor(const Eigen::MatrixXd& matrix,
								   Eigen::Tensor<double, 3>& tensor) const;

		Eigen::MatrixXd im2col(const Eigen::Tensor<double, 3>& input_tensor,
							   int filter_length, int pad_l, int stride);

		void col2im(const Eigen::MatrixXd& col_matrix, int batch_size, 
					int input_length, int input_channels, int filter_length, 
					int pad_l, int stride);

		void SetWeightMomentumsTensor(const Eigen::Tensor<double, 3>& weight_momentums);
		void SetBiasMomentumsTensor(const Eigen::VectorXd& bias_momentums);
		void SetWeightCachesTensor(const Eigen::Tensor<double, 3>& weight_caches);
		void SetBiasCachesTensor(const Eigen::VectorXd& bias_caches);

		void UpdateWeightsTensor(Eigen::Tensor<double, 3>& weight_update);
		void UpdateWeightsCacheTensor(Eigen::Tensor<double, 3>& weight_update);
		void UpdateBiasesTensor(Eigen::VectorXd& bias_update);
		void UpdateBiasesCacheTensor(Eigen::VectorXd& bias_update);
	};
} // namespace NEURAL_NETWORK

#endif // __CONVOLUTION_1D_H__
